import React, { useState } from "react";
import { Link } from "react-router-dom";
import Footer from "../../../components/Footer";

export default function GangaGhatSahibganjHome() {
  const [activeTab, setActiveTab] = useState("about");

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header with Background Image */}
      <header
        className="w-full h-96 bg-cover bg-center flex items-center justify-center text-white relative"
        style={{
          backgroundImage: "url('https://images.hindustantimes.com/rf/image_size_630x354/HT/p2/2020/07/08/Pictures/pressure-constructed-neeldhara-chandighat-haridwar-development-rameshwar_3054507a-c081-11ea-b246-8f7a5e10b5dd.jpg'), linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5))",
        }}
      >
        <div className="text-center z-10">
          <h1 className="text-4xl font-bold">Ganga Ghat Sahibganj</h1>
          <p className="text-xl mt-2">A Spiritual Haven on the Ganges</p>
        </div>
      </header>

      {/* Main Content */}
      <main className="max-w-6xl mx-auto px-6 py-10">
        <section className="bg-white p-6 rounded-lg shadow-md mb-8">
          <h2 className="text-2xl font-semibold text-green-700">Explore</h2>
          <div className="mt-4">
            <div className="flex space-x-4 mb-4">
              <button
                className={`px-4 py-2 rounded ${activeTab === "about" ? "bg-green-600 text-white" : "bg-gray-200"}`}
                onClick={() => setActiveTab("about")}
              >
                About Ganga Ghat Sahibganj
              </button>
              <button
                className={`px-4 py-2 rounded ${activeTab === "map" ? "bg-green-600 text-white" : "bg-gray-200"}`}
                onClick={() => setActiveTab("map")}
              >
                Map
              </button>
            </div>
            {activeTab === "about" && (
              <div>
                <p className="text-gray-700">
                  Ganga Ghat Sahibganj is a serene riverside ghat located along the banks of the Ganges in Sahibganj, Jharkhand. This spiritual and cultural site is a place of pilgrimage and reflection, attracting devotees and tourists alike. The ghat offers stunning views of the river, especially during sunrise and sunset, and is a hub for religious activities, including the Chhath Puja festival.
                </p>
                <p className="mt-4 text-gray-700">
                  The ghat is historically significant, with ties to ancient trade routes and local traditions. The best time to visit is from October to March when the weather is cool and the ghat is adorned with festive decorations during festivals.
                </p>
                <img
                  src="https://www.hindustantimes.com/ht-img/img/2023/05/06/550x309/Cities-of-northern-India-with-cultural-significanc_1683354813017.jpg"
                  alt="Ganga Ghat Sahibganj"
                  className="w-full h-96 object-cover rounded mt-4"
                  onError={(e) => { e.target.src = "https://via.placeholder.com/600x400?text=Ganga+Ghat+Image"; }}
                />
              </div>
            )}
            {activeTab === "map" && (
              <div>
                <p className="text-gray-700">
                  Ganga Ghat Sahibganj is located in Sahibganj town, Jharkhand, along the Ganges River. It is easily accessible within the town. Below is a map to help you locate the ghat:
                </p>
                <div className="mt-4">
                  <iframe
                    title="Ganga Ghat Sahibganj Map"
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3643.123456!2d87.645!3d25.252!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMjXCsDE1JzA5LjEiTiA4N8KwMzgnNDguNCJF!5e0!3m2!1sen!2sin!4v1693819200!5m2!1sen!2sin"
                    width="100%"
                    height="450"
                    style={{ border: 0 }}
                    allowFullScreen=""
                    loading="lazy"
                    referrerPolicy="no-referrer-when-downgrade"
                  ></iframe>
                </div>
              </div>
            )}
          </div>
        </section>

        <section className="bg-orange-100 p-6 rounded-lg shadow-md mb-8">
          <h2 className="text-2xl font-semibold text-green-700">How to Get There</h2>
          <ul className="mt-4 text-gray-700 space-y-4">
            <li className="flex items-start">
              <img
                src="https://www.gujarattourism.com/content/dam/gujrattourism/images/other-images/by-road.png"
                alt="By Road"
                className="w-16 h-16 mr-4"
                onError={(e) => { e.target.src = "https://via.placeholder.com/64?text=Road+Icon"; }}
              />
              <div>
                <strong>By Road:</strong> Ganga Ghat is centrally located in Sahibganj town and can be reached by auto-rickshaw or taxi from any part of the town.
              </div>
            </li>
            <li className="flex items-start">
              <img
                src="https://www.gujarattourism.com/content/dam/gujrattourism/images/other-images/by-train.png"
                alt="By Train"
                className="w-16 h-16 mr-4"
                onError={(e) => { e.target.src = "https://via.placeholder.com/64?text=Train+Icon"; }}
              />
              <div>
                <strong>By Train:</strong> The nearest railway station is Sahibganj Railway Station, about 2 km from the ghat. Local transport can take you there quickly.
              </div>
            </li>
            <li className="flex items-start">
              <img
                src="https://www.gujarattourism.com/content/dam/gujrattourism/images/other-images/by-air.png"
                alt="By Air"
                className="w-16 h-16 mr-4"
                onError={(e) => { e.target.src = "https://via.placeholder.com/64?text=Air+Icon"; }}
              />
              <div>
                <strong>By Air:</strong> The nearest airport is Birsa Munda Airport in Ranchi, approximately 400 km away. From Ranchi, take a train or bus to Sahibganj and then local transport to the ghat.
              </div>
            </li>
          </ul>
        </section>

        <section className="bg-white p-6 rounded-lg shadow-md mb-8">
          <h2 className="text-2xl font-semibold text-green-700">Nearby Attractions</h2>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link to="/" className="relative group block">
              <img
                src="https://www.tataneu.com/pages/travel/_next/image?url=https%3A%2F%2Fd1msew97rp2nin.cloudfront.net%2Fprodin%2Ftntravel%2Fblogimages%2Ffaqs-about-the-largest-bird-sanctuary-in-india-cadbd95d-67ea-4e14-8432-e49eaa0b6014.webp&w=3840&q=75"
                alt="Udhwa Lake Bird Sanctuary"
                className="w-full h-48 object-cover rounded"
                onError={(e) => { e.target.src = "https://via.placeholder.com/384x192?text=Bird+Sanctuary"; }}
              />
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition">
                <p className="font-bold">Udhwa Lake Bird Sanctuary</p>
              </div>
            </Link>
            <Link to="/moti-jharna" className="relative group block">
              <img
                src="https://i0.wp.com/angdesh.com/wp-content/uploads/2022/07/moti-jharna-waterfall-Sahibganj-3.jpg?w=500&ssl=1"
                alt="Moti Jharna"
                className="w-full h-48 object-cover rounded"
                onError={(e) => { e.target.src = "https://via.placeholder.com/384x192?text=Moti+Jharna"; }}
              />
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition">
                <p className="font-bold">Moti Jharna</p>
              </div>
            </Link>
            <Link to="/teliagarhi-fort" className="relative group block">
              <img
                src="https://angdesh.com/wp-content/uploads/2022/03/cropped-Vikramshila.jpg"
                alt="Teliagarhi Fort"
                className="w-full h-48 object-cover rounded"
                onError={(e) => { e.target.src = "https://via.placeholder.com/384x192?text=Teliagarhi+Fort"; }}
              />
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition">
                <p className="font-bold">Teliagarhi Fort</p>
              </div>
            </Link>
            <Link to="/rajmahal-fossil-park" className="relative group block">
              <img
                src="data:image/jpeg;base64,"
                alt="Rajmahal Fossil Park"
                className="w-full h-48 object-cover rounded"
                onError={(e) => { e.target.src = "https://via.placeholder.com/384x192?text=Rajmahal+Fossil+Park"; }}
              />
              <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition">
                <p className="font-bold">Rajmahal Fossil Park</p>
              </div>
            </Link>
          </div>
        </section>
      </main>

      <Footer />
    </div>
  );
}